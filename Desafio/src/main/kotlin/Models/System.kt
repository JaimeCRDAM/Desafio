...
class System {
    ...
    fun Simulacion(){
        ...
        var turnTime = 10
        var Days = 7
        var totalSimTime = Days*turnsPerDay*turnTime+1
        var DayOfWeek = GetCurrentDay(time, turnsPerDay*turnTime)
    ...
    private fun GetCurrentDay(time:Int, totalTurnTime:Int):Int{
        var dayNumber:Int = floor((time / totalTurnTime).toDouble()).toInt()
        if (dayNumber == 7)
            return 7
        return dayNumber+1
    }
    private fun TurnRecapToString(pair:Pair<ArrayList<String>, ArrayList<String>>, day:DayOfWeek, turnTime:Int){
        var arrayOfTimes = arrayOf("00:00-08:00", "08:00-16:00", "16:00-00:00")
